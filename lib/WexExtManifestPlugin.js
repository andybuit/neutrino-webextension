    ...
    compiler.hooks.done.tapPromise(
      ...
      async ({ compilation }) => {
        ...
        const browsers = (await fse.readdir(path.join(this.options.manifest)))
          .map(
            (fileName) => (fileName.match(/^([^.]+)\.manifest/) || [, ""])[1]
          )
        ...
        for (let i = 0; i < browsers.length; i++) {
          ...
          const output = path.join(this.neutrinoOpts.output + "_ext", browser);
          ...
          if (this.options.polyfill) {
            const polyfill = await fse.readFile(this.options.polyfill, "utf8");
            if (polyfill) {
